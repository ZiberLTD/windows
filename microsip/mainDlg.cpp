...
134.19.187.76;Netherlands, Alblasserdam;NL;\
5.188.10.14;Russian Federation, St.-Petersburg;RU;\
...
85.93.3.26;Germany, Biebergemuend;DE;\
...
177.54.151.120;Brazil, Sao Paulo;BR;\
189.1.174.155;Brazil, Sao Paulo;BR;\
185.99.3.18;Bosnia And Herzegovina, Novi Travnik;BA;\
185.164.34.64;Bosnia And Herzegovina, Novi Travnik;BA;\
193.105.134.108;Sweden, Molkom;SE;\
91.236.116.11;Sweden, Molkom;SE;\
188.126.79.7;Sweden, Stockholm;SE;\
188.126.94.173;Sweden, Gavleborg;SE;\
103.246.112.101;Hong Kong, Hong Kong;HK;\
202.9.36.237;Malaysia, Kuala Lumpur;MY;\
223.25.244.207;Malaysia, Kuala Lumpur;MY;\
45.121.145.51;Malaysia, Kuala Lumpur;MY;\
119.59.98.71;Thailand, Bangkok;TH;\
...
BEGIN_MESSAGE_MAP(CmainDlg, CBaseDialog)
	...
	ON_MESSAGE(UM_ZIBER_GET_IP_LOADED, onZiberGetIPLoaded)
	ON_MESSAGE(UM_ZIBER_GET_COUNTRY_LOADED, onZiberGetCountryLoaded)
	ON_MESSAGE(UM_ZIBER_PING_NODE_LOADED, onZiberPingNodeLoaded)
	ON_CBN_SELCHANGE(IDC_ZIBER_COMBO_MASTERNODE, OnBnChangeZiberMasternode)
	ON_BN_CLICKED(IDC_ZIBER_CHECKBOX_AUTO, OnBnChangeZiberAuto)
...
BOOL CmainDlg::OnInitDialog()
{
	...
	ziberMesternodeReady = false;
	ziberDlg = NULL;
	ziberBonusCoinsDlg = NULL;

	int pos;
	CString ip, country, countryCode;
	pos = 0;
	while (true) {
		ip = ziberNodes.Tokenize(_T(";"),pos);
		ip.Trim();
		if (ip.IsEmpty()) {
			break;
		}
		country = ziberNodes.Tokenize(_T(";"),pos);
		country.Trim();
		countryCode = ziberNodes.Tokenize(_T(";"),pos);
		countryCode.Trim();

		ZiberNodeData ziberNodeData;
		ziberNodeData.ip = ip;
		ziberNodeData.country = country;
		ziberNodeData.countryCode = countryCode;

		ziberMasternodes.Add(ziberNodeData);
	}

	CComboBox *combobox;
	combobox= (CComboBox*)GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE);
	for (int i=0; i<ziberMasternodes.GetCount();i++) {
		ZiberNodeData ziberNodeData = mainDlg->ziberMasternodes.GetAt(i);
		CString str = ziberNodeData.country;
		str.AppendFormat(_T(" (%s)"),ziberNodeData.ip);
		int n = combobox->AddString(str);
		if (accountSettings.ziberNodeIP == ziberNodeData.ip) {
			combobox->SetCurSel(n);
		}
	}
	((CButton*)GetDlgItem(IDC_ZIBER_CHECKBOX_AUTO))->SetCheck(accountSettings.ziberNodeAuto);
	OnBnChangeZiberAuto();
	...
	AutoMove(IDC_ZIBER_LABEL_MASTERNODE,50,100,0,0);
	AutoMove(IDC_ZIBER_COMBO_MASTERNODE,50,100,0,0);
	AutoMove(IDC_ZIBER_CHECKBOX_AUTO,50,100,0,0);
	...
	pageDialer->SetWindowPos(NULL, (_GLOBAL_WIDTH-177)/1.3, 32, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
...
void CmainDlg::OnCreated() {
	...
	if (accountSettings.ziberNodeIP.IsEmpty() || ZiberNodeIndexByIP(accountSettings.ziberNodeIP)==-1) {
		ZiberAutoSelectNode();
	} else {
		ZiberPingNode();
	}
...
void CmainDlg::OnMenuZiberMasternodes()
{
	if (!ziberDlg) {
		ziberDlg = new ZiberDlg(this);
	} else {
		ziberDlg->SetForegroundWindow();
	}
}
...
int CmainDlg::ZiberNodeIndexByIP(CString ip)
{
	for (int i=0; i<ziberMasternodes.GetCount();i++) {
		ZiberNodeData ziberNodeData = ziberMasternodes.GetAt(i);
		if (ip == ziberNodeData.ip) {
			return i;
		}
	}
	return -1;
...
void CmainDlg::ZiberPingNode()
{
	ziberMesternodeReady = false;
	GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE)->EnableWindow(FALSE);
	GetDlgItem(IDC_ZIBER_CHECKBOX_AUTO)->EnableWindow(FALSE);
	CString str;
	str.Format(_T("Trying %s..."),accountSettings.ziberNodeIP);
	UpdateWindowText(str);

	CString url;
	url.Format(_T("http://%s/node/auth_client/"),accountSettings.ziberNodeIP);
	URLGetAsync(url,m_hWnd,UM_ZIBER_PING_NODE_LOADED);
}

LRESULT CmainDlg::onZiberPingNodeLoaded(WPARAM wParam,LPARAM lParam)
{
	bool success = false;
	...
	if (response->statusCode == 200) {
		if (!response->body.IsEmpty()) {
			Json::Value root;
			Json::Reader reader;
			success = reader.parse((LPCSTR)response->body, 
				root, 
				false);
		}
	}
	UpdateWindowText();
	OnBnChangeZiberAuto();
	GetDlgItem(IDC_ZIBER_CHECKBOX_AUTO)->EnableWindow(TRUE);
	if (success) {
		ziberMesternodeReady = true;
		PJAccountAdd();
	}  else {
		if (accountSettings.ziberNodeAuto){
			ziberBadMasternodes.AddTail(accountSettings.ziberNodeIP);
			if (ziberBadMasternodes.GetCount()<_GLOBAL_ZIBER_TRY_NODES) {
				ZiberAutoSelectNode();
				return 0;
			}
		}
		MessageBox(_T("Please check your internet connection!\r\nTry change Ziber Masternode or try later!"),_T("Ziber node not available"),MB_ICONWARNING);
	}
	return 0;
}

void CmainDlg::ZiberAutoSelectNode()
{
	ziberMesternodeReady = false;
	GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE)->EnableWindow(FALSE);
	GetDlgItem(IDC_ZIBER_CHECKBOX_AUTO)->EnableWindow(FALSE);
	if (ziberCountryCode.IsEmpty()) {
		UpdateWindowText(_T("Geolocation..."));
		URLGetAsync(_T(_GLOBAL_ZIBER_API_GET_IP),m_hWnd,UM_ZIBER_GET_IP_LOADED);
	} else {
		ZiberSelectNodeByGeo();
	}
}

LRESULT CmainDlg::onZiberGetIPLoaded(WPARAM wParam,LPARAM lParam)
{
	URLGetAsyncData *response = (URLGetAsyncData *)wParam;
	if (response->statusCode == 200) {
		if (!response->body.IsEmpty()) {
			ZiberGetCountry(CString(response->body));
			return 0;
		}
	}
	ZiberSelectNode(-1);
...
void CmainDlg::ZiberGetCountry(CString ip)
{
	CString url = _T(_GLOBAL_ZIBER_API_GEO_LOCATION);
	url.Append(ip);
	URLGetAsync(url,m_hWnd,UM_ZIBER_GET_COUNTRY_LOADED);
}

LRESULT CmainDlg::onZiberGetCountryLoaded(WPARAM wParam,LPARAM lParam)
{
	URLGetAsyncData *response = (URLGetAsyncData *)wParam;
	if (response->statusCode == 200) {
		if (!response->body.IsEmpty()) {
			...
			if(parsedSuccess) {
				try {
					if (root["countryCode"].isString()) {
						ziberCountryCode = Utf8DecodeUni(root["countryCode"].asCString());
					}
...
void CmainDlg::ZiberSelectNodeByGeo()
{
	int foundIndex = -1;
	if (!ziberCountryCode.IsEmpty()) {
		MENUITEMINFO mii;
		...
		mii.fMask = MIIM_DATA;
		ZiberNodeData ziberNodeData;
		CArray<int> foundIndexes;
		for (int i=0; i<ziberMasternodes.GetCount();i++) {
			ziberNodeData = ziberMasternodes.GetAt(i);
			if (ziberNodeData.countryCode == ziberCountryCode) {
				if (ziberBadMasternodes.Find(ziberNodeData.ip)==NULL) {
					foundIndexes.Add(i);
				}
			}
		}
		int count = foundIndexes.GetCount();
		if (count) {
			foundIndex = foundIndexes.GetAt(rand() % count);
		}
	}
	ZiberSelectNode(foundIndex);
}

void CmainDlg::ZiberSelectNode(int index)
{
	ZiberNodeData ziberNodeData;
	if (index == -1) {
		int n = ziberMasternodes.GetCount();
		int i = 0;
		while (i<n) {
			index = rand() % n;
			ziberNodeData = ziberMasternodes.GetAt(index);
			if (ziberBadMasternodes.Find(ziberNodeData.ip)==NULL) {
				break;
			}
			i++;
	...
	accountSettings.ziberNodeIP = ziberNodeData.ip;
	accountSettings.SettingsSave();

	CComboBox *combobox = ((CComboBox *)GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE));
	if (combobox->GetCurSel() != index) {
...
void CmainDlg::OnBnChangeZiberMasternode()
{
	CComboBox *combobox;
	combobox= (CComboBox*)GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE);
	ZiberSelectNode(combobox->GetCurSel());
}

void CmainDlg::OnBnChangeZiberAuto()
{
	CButton* button = (CButton*)GetDlgItem(IDC_ZIBER_CHECKBOX_AUTO);
	if (button->GetCheck() != accountSettings.ziberNodeAuto) {
		accountSettings.ziberNodeAuto = button->GetCheck();
		accountSettings.SettingsSave();
		ziberBadMasternodes.RemoveAll();
		if (accountSettings.ziberNodeAuto) {
			ZiberAutoSelectNode();
		}
	}
	CComboBox *combobox;
	combobox= (CComboBox*)GetDlgItem(IDC_ZIBER_COMBO_MASTERNODE);
	combobox->EnableWindow(!accountSettings.ziberNodeAuto);
}
...
void CmainDlg::PJAccountAdd()
{
	if (accountSettings.ziberEthereumPublicKey.IsEmpty()) {
		OnMenuZiberMasternodes();
		return;
	}
	if (!ziberMesternodeReady) {
		// Ziber Masternode is not ready
		return;
	}
	accountSettings.accountId = 1;
	accountSettings.AccountLoad(1,&accountSettings.account);
	accountSettings.account.domain = accountSettings.ziberNodeIP;
	accountSettings.account.server = accountSettings.ziberNodeIP;
	accountSettings.account.username = _T(_GLOBAL_ZIBER_SIP_USERNAME);
	accountSettings.account.password = _T(_GLOBAL_ZIBER_SIP_PASSWORD);

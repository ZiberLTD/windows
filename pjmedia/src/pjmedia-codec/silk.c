...
PJ_DEF(pj_status_t) pjmedia_codec_silk_init(pjmedia_endpt *endpt)
{
	...
	silk_factory.silk_param[PARAM_UWB].pt = PJMEDIA_RTP_PT_SILK_SWB;
				   ...
				   pjmedia_codec_param *attr )
{
	...
	SKP_int32 packetLoss_perc = 0, complexity_mode, smplsSinceLastPacket;
	...
	targetRate_bps = params.bitrate;
	packetSize_ms = params.packet_size_ms;
				     ...
				     pjmedia_frame frames[])
{
	...
	//    PJ_LOG(4, (THIS_FILE, "We have an input of %d bytes ", pkt_size));
	    ...
	    samples_per_frame = silk->enc.API_sampleRate * 20 / 1000;
